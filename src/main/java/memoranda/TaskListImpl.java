...
public class TaskListImpl implements ITaskList {
    ...
    public boolean hasSubTasks(String id) {
        ...
        if(task != null && task.getChildElements("task").size() > 0) {
            return true;
        }
        else {
            return false;
    ...
    /**
     * corrects any inconsistencies in start dates
     * 
     * @param t
     * @return
     */
    public CalendarDate getEarliestStartDateFromSubTasks(ITask t) {
    	return getDateFromSubTasks(t, true);
    }
    ...
    public CalendarDate getLatestEndDateFromSubTasks(ITask t) {
    	return getDateFromSubTasks(t, false);
    }
  ...
  *  
  *  fixed the two methods above this one that had duplicate code.
  *  In class TaskListImpl.java, I noticed the methods
  *  getEarliestStartDateFromDubTasks(ITask t) and
  *  getLatestEndDateFromDubTasks(ITask t) had duplicate code.
  *  They were almost the exact same method, other than one line stating
  *  dd.before(d) and the other stating dd.after(d).  The other different
  *  was the setter and getter in each method getting either the StartDate or Enddate.
  *  I made one new method called getDateFromSubTasks that each of the original methods now calls.
  *   
  * Looks through the entire sub task tree and corrects any inconsistencies in start dates/end dates
  * 
  * @param t
  * @param earliest
  * @return
  */
 private CalendarDate getDateFromSubTasks(ITask t, Boolean earliest) {
     CalendarDate d = earliest ? t.getStartDate() : t.getEndDate();
     if (hasSubTasks(t.getID())) {
	        Collection subTasks = getAllSubTasks(t.getID());
	        for (Iterator iter = subTasks.iterator(); iter.hasNext();) {
	        	ITask e = (ITask) iter.next();
	        	CalendarDate dd = getEarliestStartDateFromSubTasks(e);
	        	if(earliest ? dd.before(d) : dd.after(d)) {
	        	    d = dd;
	        	}
	        }
	        if (earliest){
         ...
         return earliest ? t.getStartDate() : t.getEndDate();
     }
 }
